...
The central server is implemented using Flask and Flask-SocketIO~\cite{flaskSocketIO}. It exposes:
\begin{itemize}
  \item Socket.IO namespaces for receiving real-time metrics and broadcasting updates to dashboard clients.
  \item RESTful HTTP endpoints for historical data retrieval and configuration management.
  \item A PostgreSQL database for persistent logging with retention policies.
\end{itemize}
...
This event-driven pattern reduces latency compared to polling and leverages the low-latency, bidirectional communication properties of Socket.IO~\cite{demirSocketIO,socketioDocs}.
...
\end{equation}
assuming all units are online.

To avoid unbounded growth, a retention policy is enforced. For example, data older than $R$ days can be purged periodically using a scheduled cleanup routine:
\begin{equation}
  t_{\text{expire}} = t_{\text{now}} - R \cdot 24 \times 3600.
\end{equation}

This approach balances historical analysis capability with storage constraints and remains suitable for small- to medium-scale deployments.
...
\begin{table}[H]
  ...
  \begin{tabular}{lcc}
    ...
    SQLite entries/day    & $\approx 100\,000$ & All units combined \\
...
The results indicate that the agent has a negligible impact on host performance, and the server scales linearly with the number of units under the tested conditions. PostgreSQL sustained the daily write volume without noticeable degradation in query latency for recent data.
