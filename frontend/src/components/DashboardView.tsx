...
export default function DashboardView({ orgId }: DashboardViewProps) {
    const { data, loading, error, refetch, filteredData, setSelectedUnitId } = useUsageData(orgId)
    const { units, loading: unitsLoading } = useUnits(orgId)
            ...
            <div className="min-h-screen bg-slate-900 flex">
                ...
                <div className="flex-1 ml-64 flex items-center justify-center">
                    <div className="text-center">
                        ...
                        <p className="text-slate-400 text-sm">Loading {orgId ? `${orgId} ` : ''}system data</p>
                    </div>
                </div>
            </div>
        )
            ...
            <div className="w-64 bg-slate-800 border-r border-slate-700 flex flex-col fixed h-screen">
                ...
                <div className="p-6 border-b border-slate-700">
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            <h1 className="text-lg font-bold text-white">{orgId ? orgId : 'Lab Monitoring'}</h1>
                ...
                <div className="p-4 border-b border-slate-700">
                    <div className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">Computers</div>
                    <div className="space-y-2">
                        <button
                            ...
                            className={`w-full px-4 py-3 rounded-lg transition-colors text-left flex items-center space-x-3 ${selectedUnit === null
                        ...
                        {units.map((unit) => (
                            <button
                                ...
                                className={`w-full px-4 py-3 rounded-lg transition-colors text-left flex items-center space-x-3 ${selectedUnit?.id === unit.id
                                    ? 'bg-blue-600 text-white'
                                    : 'text-slate-300 hover:bg-slate-700'
                                    }`}
                            >
                ...
                <div className="p-4 overflow-y-auto flex-1">
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="bg-slate-700/50 rounded-lg p-3 border border-slate-600">
                            ...
                            <div className="text-xl font-semibold text-white mb-1">{units.filter(u => u.status === 'online').length} / {units.length}</div>
                ...
                <div className="max-w-7xl mx-auto px-6 py-3">
                    <div className="space-y-3">
                        ...
                        <div className="bg-slate-800 border border-slate-700 rounded">
                            <div className="px-4 py-2 border-b border-slate-700 bg-slate-800/50">
                                <div className="flex items-center justify-between">
                                    <h2 className="text-sm font-semibold text-white">GPU Usage</h2>
                                    <span className="text-xs text-slate-400">
                                        {currentData ? `${(currentData.gpu_load || 0).toFixed(1)}%` : '0%'}
                                    </span>
                                </div>
